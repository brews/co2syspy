...
def CO2SYS(PAR1, PAR2, PAR1TYPE, PAR2TYPE, SAL, TEMPIN, TEMPOUT, PRESIN, PRESOUT, SI, PO4, pHSCALEIN, K1K2CONSTANTS, KSO4CONSTANTS):
    #**************************************************************************
    #
    ...
    PAR1TYPE = np.atleast_1d(PAR1TYPE)
    PAR2TYPE = np.atleast_1d(PAR2TYPE)
    SAL = np.atleast_1d(SAL)
    TEMPIN = np.atleast_1d(TEMPIN)
    TEMPOUT = np.atleast_1d(TEMPOUT)
    PRESIN = np.atleast_1d(PRESIN)
    PRESOUT = np.atleast_1d(PRESOUT)
    SI = np.atleast_1d(SI)
    PO4 = np.atleast_1d(PO4)
    pHSCALEIN = np.atleast_1d(pHSCALEIN)
    K1K2CONSTANTS = np.atleast_1d(K1K2CONSTANTS)
    KSO4CONSTANTS = np.atleast_1d(KSO4CONSTANTS)

    veclengths= {len(PAR1), len(PAR2), len(PAR1TYPE), len(PAR2TYPE), len(SAL), len(TEMPIN), len(TEMPOUT), len(PRESIN),
                 len(PRESOUT), len(SI), len(PO4), len(pHSCALEIN), len(K1K2CONSTANTS), len(KSO4CONSTANTS)}

    if len(veclengths) > 2:
        print('*** INPUT ERROR: Input vectors must all be of same length, or of length 1. ***')
        return

    # Find longest column vector
    ntps = max(veclengths)
    ...
    F = p1 == 1
    TA[F] = PAR1[F] / 1e6  # micromol/kg to mol/kg
    F = p1 == 2
    TC[F] = PAR1[F] / 1e6  # micromol/kg to mol/kg
    F = p1 == 3
    PH[F] = PAR1[F]
    F = p1 == 4
    PC[F] = PAR1[F] / 1e6  # microatm. to atm.
    F = p1 == 5
    FC[F] = PAR2[F] / 1e6  # microatm. to atm.
    F = p2 == 1
    TA[F] = PAR2[F] / 1e6  # micromol/kg to mol/kg
    F = p2 == 2
    TC[F] = PAR2[F] / 1e6  # micromol/kg to mol/kg
    F = p2 == 3
    PH[F] = PAR2[F]
    F = p2 == 4
    PC[F] = PAR2[F] / 1e6  # microatm. to atm.
    F = p2 == 5
    FC[F] = PAR2[F] / 1e6  # microatm. to atm.
    ...
    Icase = 10 * np.min([p1.min(), p2.min()]) + np.max([p1.max(), p2.max()])
    ...
    F = Icase == 14 or Icase == 15  # input TA, (pCO2 or fCO2)
    if np.any(F):
    ...
    if np.any(F):
        ...
        TAc[F] = CalculateTAfromTCpH(TCc[F], PHic[F]) + PengCorrection[F]
    F = Icase == 34 or Icase == 35  # input pH, (pCO2 or fCO2)
    if np.any(F):
    ...
    HCO3inp, CO3inp, BAlkinp, OHinp, PAlkinp, SiAlkinp, Hfreeinp, HSO4inp, HFinp = CalculateAlkParts(PHic, TCc)
    PAlkinp = PAlkinp + PengCorrection
    CO2inp = TCc - CO3inp - HCO3inp
    F = np.ones(ntps, dtype=bool)  # i.e., do for all samples:
    ...
    KIVEC = [K0, K1, K2 - np.log10(K1) - np.log10(K2), KW, KB, KF, KS, KP1, KP2, KP3, KSi]
    ...
    HCO3out, CO3out, BAlkout,OHout, PAlkout, SiAlkout, Hfreeout, HSO4out, HFout = CalculateAlkParts(PHoc, TCc)
    ...
    KOVEC = np.array([K0, K1, K2, -np.log10(K1) - np.log10(K2), KW, KB, KF, KS, KP1, KP2, KP3, KSi])
    TVEC = np.array([TB, TF, TS])


    # Saving data in array, 81 columns, as many rows as samples input
    # Multiplied Hfreeinp *1e6, svh20100827
    # Multiplied Hfreeout *1e6, svh20100827

    DATA = [TAc * 1e6, TCc * 1e6, PHic, PCic * 1e6, FCic * 1e6,
           HCO3inp * 1e6, CO3inp * 1e6, CO2inp * 1e6, BAlkinp * 1e6, OHinp * 1e6,
           PAlkinp * 1e6, SiAlkinp * 1e6, Hfreeinp * 1e6, Revelleinp, OmegaCainp, 
           OmegaArinp, xCO2dryinp * 1e6, PHoc, PCoc * 1e6, FCoc * 1e6,
           HCO3out * 1e6, CO3out * 1e6, CO2out * 1e6, BAlkout * 1e6, OHout * 1e6,
           PAlkout * 1e6, SiAlkout * 1e6, Hfreeout * 1e6, Revelleout, OmegaCaout, 
           OmegaArout, xCO2dryout * 1e6, pHicT, pHicS, pHicF,
           pHicN, pHocT, pHocS, pHocF, pHocN, TEMPIN, TEMPOUT, PRESIN, PRESOUT, PAR1TYPE,
           PAR2TYPE, K1K2CONSTANTS, KSO4CONSTANTS, pHSCALEIN, SAL, PO4, SI, KIVEC, KOVEC,
           TVEC * 1e6]
    ...
    return out, NICEHEADERS


def Constants(TempC, Pdbar):
    ...
    if np.any(F):
        ...
        FF = F and (WhoseKSO4 == 3 or WhoseKSO4 == 4)  # if user opted for the new Lee values:
        if np.any(FF):
    ...
    TempK100 = TempK / 100
    lnK0 = -60.2409 + 93.4517 / TempK100 + 23.3585 * np.log(TempK100) + Sal * (0.023517 - 0.023656 * TempK100 + 0.0047036 * TempK100 ** 2)
    K0 = np.exp(lnK0)
    ...
    if np.any(F):
        ...
        lnKS[F] = -4276.1 / TempK[F] + 141.328 - 23.093 * logTempK[F] + (-13856 / TempK[F] + 324.57 - 47.986 * logTempK[F]) * np.sqrt(IonS[F]) + (35474 / TempK[F] - 771.54 + 114.723 * logTempK[F]) * IonS[F] + (-2698 / TempK[F]) * np.sqrt(IonS[F]) * IonS[F] + (1776 / TempK[F]) * IonS[F] ** 2 
        KS[F] = np.exp(lnKS[F]) * (1 - 0.001005 * Sal[F])  # this first multiple is on the free pH scale in mol/kg-H2O; then convert to mol/kg-SW
    F = WhoseKSO4 == 2 or WhoseKSO4 == 4
    if np.any(F):
        ...
        pKS[F] = 647.59 / TempK[F] - 6.3451 + 0.019085 * TempK[F] - 0.5208 * np.sqrt(IonS[F])
        KS[F] = 10 ** -pKS[F] * (1 - 0.001005 * Sal[F])  # this first multiple is on the free pH scale in mol/kg-H2O; then convert to mol/kg-SW
    ...
    F = WhichKs == 7
    if np.any(F):
        fH[F] = 1.29 - 0.00204 * TempK[F] + (0.00046 - 0.00000148 * TempK[F]) * Sal[F] * Sal[F]
    ...
    F = WhichKs != 7 and WhichKs != 8
    if np.any(F):
        fH[F] = 1.2948 - 0.002036 * TempK[F] + (0.0004607 - 0.000001475 * TempK[F]) * Sal[F] ** 2
    ...
    if np.any(F):
        ...
        logKB[F] = -9.26 + 0.00886 * Sal[F] + 0.01 * TempC[F]
        KB[F] = 10 **(logKB[F]) / fH[F]  # Numerator is on NBS scale; denominator is converted to SWS scale
    F = WhichKs != 6 and WhichKs != 7 and WhichKs != 8
    if np.any(F):
        # Dickson, A. G., Deep-Sea Research 37:755-766, 1990:
        lnKBtop[F] = -8966.9 - 2890.53 * sqrSal[F] - 77.942 * Sal[F] + 1.728 * sqrSal[F] * Sal[F] - 0.0996 * Sal[F] ** 2
        lnKB[F] = lnKBtop[F] / TempK[F] + 148.0248 + 137.1942 * sqrSal[F] + 1.62142 * Sal[F] + (-24.4344 - 25.085 * sqrSal[F] - 0.2474 * Sal[F]) * logTempK[F] + 0.053105 * sqrSal[F] * TempK[F]
        KB[F] = np.exp(lnKB[F]) / SWStoTOT[F]  # Numerator is on total pH scale in mol/kg-SW; denominator convert to SWS pH scale
    ...
    if np.any(F):
        ...
        lnKW[F] = 148.9802 - 13847.26 / TempK[F] - 23.6521 * logTempK[F] + (-79.2447 + 3298.72 / TempK[F] + 12.0408 * logTempK[F]) * sqrSal[F] - 0.019813 * Sal[F]
    F = WhichKs == 8
    if np.any(F):
        # Millero, Geochemica et Cosmochemica Acta 43:1651-1661, 1979
    ...
    if np.any(F):
        ...
        lnKW[F] = 148.9802 - 13847.26 / TempK[F] - 23.6521 * logTempK[F] + (-5.977 + 118.67 / TempK[F] + 1.0495 * logTempK[F]) * sqrSal[F] - 0.01615 * Sal[F]
    ...
    if np.any(F):
        ...
        KP2[F] = np.exp(-9.039 - 1450 / TempK[F]) / fH[F]  # Numerator is on NBS scale; denominator convert to SWS scale
        KP3[F] = np.exp(4.466 - 7276 / TempK[F]) / fH[F]  # Numerator is on NBS scale; denominator convert to SWS scale
        ...
        KSi[F] = 0.0000000004 / fH[F]  # Numerator is NBS scale; denominator convert to SWS scale
    F = WhichKs == 6 or WhichKs == 8
    if np.any(F):
        KP1[F] = 0
        KP2[F] = 0
        KP3[F] = 0
    ...
    if np.any(F):
        ...
        lnKP1[F] = -4576.752 / TempK[F] + 115.54 - 18.453 * logTempK[F] + (-106.736 / TempK[F] + 0.69171) * sqrSal[F] + (-0.65643 / TempK[F] - 0.01844) * Sal[F]
        KP1[F] = np.exp(lnKP1[F])
        lnKP2[F] = -8814.715 / TempK[F] + 172.1033 - 27.927 * logTempK[F] + (-160.34 / TempK[F] + 1.3566) * sqrSal[F] + (0.37335 / TempK[F] - 0.05778) * Sal[F]
        KP2[F] = np.exp(lnKP2[F])
        lnKP3[F] = -3070.75 / TempK[F] - 18.126 + (17.27039 / TempK[F] + 2.81197) * sqrSal[F] + (-44.99486 / TempK[F] - 0.09984) * Sal[F]
        KP3[F] = np.exp(lnKP3[F])
        lnKSi[F] = -8904.2 / TempK[F] + 117.4 - 19.334 * logTempK[F] + (-458.79 / TempK[F] + 3.5913) * np.sqrt(IonS[F]) + (188.74 / TempK[F] - 1.5998) * IonS[F] + (-12.1652 / TempK[F] + 0.07871) * IonS[F]**2
        KSi[F] = np.exp(lnKSi[F]) * (1 - 0.001005 * Sal[F])  # this is on the SWS pH scale in mol/kg-H2O; convert to mol/kg-SW
    ...
    if np.any(F):
        ...
        lnK1[F] = 2.83655 - 2307.1266 / TempK[F] - 1.5529413 * logTempK[F] + (-0.20760841 - 4.0484 / TempK[F]) * sqrSal[F] + 0.08468345 * Sal[F] - 0.00654208 * sqrSal[F] * Sal[F]
        K1[F] = (np.exp(lnK1[F])        # this is on the total pH scale in mol/kg-H2O
        ...
        K2[F] = (np.exp(lnK2[F])         # this is on the total pH scale in mol/kg-H2O
            * (1 - 0.001005 * Sal[F])    # convert to mol/kg-SW
            / SWStoTOT[F])               # convert to SWS pH scale
    F = WhichKs == 2
    if np.any(F):
    ...
    if np.any(F):
        ...
        pK2[F] = (5371.9645 + 1.671221 * TempK[F] + 0.22913 * Sal[F] + 18.3802 * np.log10(Sal[F])
                  ...
                  - 5617.11 * np.log10(Sal[F]) / TempK[F] + 2.136 * Sal[F] / TempK[F]) # pK2 is not defined for Sal=0, since log10(0)=-inf
    ...
    if np.any(F):
        ...
        pK1[F] = 3633.86 / TempK[F] - 61.2172 + 9.6777 * np.log(TempK[F]) - 0.011555 * Sal[F] + 0.0001152 * Sal[F] ** 2
        K1[F]  = (10 ** -pK1[F]           # this is on the total pH scale in mol/kg-SW
                  / SWStoTOT[F])          # convert to SWS pH scale
        pK2[F] = 471.78 / TempK[F] + 25.929 -3.16967 * np.log(TempK[F]) - 0.01781 * Sal[F] + 0.0001122 * Sal[F] ** 2
        K2[F]  = (10 ** -pK2[F]           # this is on the total pH scale in mol/kg-SW
                  / SWStoTOT[F])          # convert to SWS pH scale
    F = WhichKs == 11
    if np.any(F): # STOPPED at ln 1032
        ...
        pK1 =  -43.6977 - 0.0129037 * Sal[F] + 1.364e-4 * Sal[F] ** 2 + 2885.378 / TempK[F] + 7.045159 * np.log(TempK[F])
        pK2 = (-452.0940 + 13.142162 * Sal[F] - 8.101e-4 * Sal[F] ** 2 + 21263.61 / TempK[F] + 68.483143 * np.log(TempK[F])
               + (-581.4428 * Sal[F] + 0.259601 * Sal[F] ** 2) / TempK[F] - 1.967035 * Sal[F] * np.log(TempK[F]))
    ...
    if np.any(F):
        ...
        pK1_0 = -126.34048 + 6320.813 / TempK[F] + 19.568224 * np.log(TempK[F])
        A_1   = 13.4191 * Sal[F] ** 0.5 + 0.0331 * Sal[F] - 5.33e-5 * Sal[F] ** 2
        B_1   = -530.123 * Sal[F] ** 0.5 - 6.103 * Sal[F]
        C_1   = -2.06950 * Sal[F] ** 0.5
        pK1[F] = A_1 + B_1 / TempK[F] + C_1 * np.log(TempK[F]) + pK1_0 # pK1 sigma = 0.0054
        K1[F] = 10 ** -pK1[F]
        pK2_0= -90.18333 + 5143.692 / TempK[F] + 14.613358 * np.log(TempK[F])    
        A_2   = 21.0894 * Sal[F] ** 0.5 + 0.1248 * Sal[F] - 3.687e-4 * Sal[F] ** 2
        B_2   = -772.483 * Sal[F] ** 0.5 - 20.051 * Sal[F]
        C_2   = -3.3336 * Sal[F] ** 0.5
        pK2[F] = A_2 + B_2 / TempK[F] + C_2 * np.log(TempK[F]) + pK2_0 # pK2 sigma = 0.011
        K2[F] = 10 ** -pK2[F]
    F = WhichKs == 14
    if np.any(F):
    ...
    deltaV = np.empty(ntps)
    deltaV[:] = np.nan
    Kappa = np.empty(ntps)
    Kappa[:] = np.nan
    lnK1fac = np.empty(ntps)
    lnK1fac[:] = np.nan
    lnK2fac = np.empty(ntps)
    lnK2fac[:] = np.nan
    lnKBfac = np.empty(ntps)
    lnKBfac[:] = np.nan
    ...
    F = WhichKs==6 or WhichKs==7 # GEOSECS and Peng assume pCO2 = fCO2, or FugFac = 1
    FugFac[F] = 1
    # CalculateVPFac:
...
def CalculatepHfCO2fromTATC(TAx, TCx):
...
def CalculatepHfromTATC(TAx, TCx):
    #Outputs pH
    ...
    while np.any(np.abs(deltapH) > pHTol):
        ...
        HSO4 = TSF / (1 + KSF / Hfree)  # Since KS is on free scale
        HF = TFF / (1 + KFF / Hfree)  # Since KF is on free scale
...
def CalculatefCO2fromTCpH(TCx, pHx):
...
def CalculateTCfromTApH(TAx, pHx):
    ...
    SiAlk = TSiF * KSiF / (KSiF + H)
    FREEtoTOT = (1 + TSF / KSF)  # pH scale conversion factor
    Hfree = H / FREEtoTOT  # for H on total scale
    HSO4 = TSF / (1 + KSF / Hfree)  # Since KS is on free scale
    HF = TFF / (1 + KFF / Hfree)  # Since KF is on free scale
...
def CalculateTAfromTCpH(TCi, pHi):
    ...
    FREEtoTOT = 1 + (TSF / KSF)  # pH scale conversion factor
    Hfree = H / FREEtoTOT  # for H on total scale
    HSO4 = TSF / (1 + KSF / Hfree)  # since KS is on free sclae
    HF = TFF / (1 + KFF / Hfree)  # since KF is on the free scale
...
def CalculatepHfromTCfCO2(TCi, fCO2i):
    ...
    H = 0.5 * (K1[F] * RR + np.sqrt(Discr)) / (1 - RR)
    pHctemp = np.log(H) / np.log(0.1)

    return pHctemp


def CalculateTCfrompHfCO2(pHi, fCO2i):
...
def CalculateAlkParts(pHx, TCx):
    ...
    Hfree = H / FREEtoTOT  # for H on total scale
    HSO4 = TS / (1 + KS / Hfree)  # since KS is on free scale
    HF = TF / (1 + KF / Hfree)  # since KF is on free scale

    return HCO3, CO3, BAlk, OH, PAlk, SiAlk, Hfree, HSO4, HF


def CaSolubility(Sal, TempC, Pdbar, TC, pH):
    # '***********************************************************************
    ...
    # Note that dic(aka TC) needs to be in mol/kg, *not* micromol/kg.
    # MATLAB has K1 K2 TempK logTempK sqrSal Pbar RT WhichKs ntps as global.
    ...
    if np.any(F):
        ...
        logKCa = logKCa + 71.595 * logTempK / np.log(10)
        ...
        logKAr = logKAr + 71.595 * logTempK / np.log(10)
    ...
    if np.any(F):
        ...
        KCa[F] = 0.0000001 * (-34.452 - 39.866 * Sal[F] ** (1 / 3) + 110.21 * np.log(Sal[F]) / np.log(10) - 0.0000075752 * TempK[F] ** 2)
        ...
        KCa[F] = KCa[F] * np.exp((36 - 0.2 * TempC[F]) * Pbar[F] / RT[F])
        KAr[F] = KAr[F] * np.exp((33.3 - 0.22 * TempC[F]) * Pbar[F] / RT[F])

    #CalculateOmegasHere:
    H = 10 ** -pH
    CO3 = TC * K1 * K2 / (K1 * H + H * H + K1 * K2)  # I think we have a float-precision problem here in comparison with MATLAB.

    omega_ca = CO3 * Ca / KCa  # OmegaCa, dimensionless
    omega_ar = CO3 * Ca / KAr  # OmegaAr, dimensionless

    return omega_ca, omega_ar


def FindpHOnAllScales(pH):
    ...
    # pHsws
    F = pHScale == 2
    factor[F] = -np.log(SWStoTOT[F]) / np.log(0.1)

    # pHfree
    F = pHScale == 3
    factor[F] = -np.log(FREEtoTOT[F]) / np.log(0.1)

    # pHNBS
    F = pHScale == 4
    factor[F] = -np.log(SWStoTOT[F]) / np.log(0.1) + np.log(fH[F]) / np.log(0.1)
